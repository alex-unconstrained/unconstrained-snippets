...
export default function CodeBlock({ html, rawCode, language }: CodeBlockProps) {
  ...
  return (
    <div className="rounded-lg border border-surface-700 overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-4 py-2.5 bg-surface-800 border-b border-surface-700">
        <span className="text-surface-500 text-xs uppercase tracking-widest">
          {language}
        </span>
        <button
          onClick={handleCopy}
          className="shrink-0 bg-brand-500 hover:bg-brand-600 text-brand-950 px-4 py-2 rounded-md text-sm font-semibold tracking-wide transition-colors cursor-pointer"
        >
